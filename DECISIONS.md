...
## 2026-06-11 — v6 LoRA recipe: Sliders-validated baseline + zero-init-escape, drop AdaLN-as-target

- **Choice:** EPOCHS 6→40 (→ ~2000 steps), LR_LORA 1e-4→2e-4, LEVEL_LR 5e-4→2e-3, LoRA rank 16→4, alpha 16→1 (α/r=0.25), drop `attn_norm.linear` from `target_modules` (keep `to_q/to_k/to_v/to_out.0`). Add per-level loss + grad-ratio diagnostics.
